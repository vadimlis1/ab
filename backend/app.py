...
try:
    ...
    # Путь к директории frontend относительно этого файла
    ...
    def api_sample_size():
        ...
        baseline_cr = float(data["baseline_cr"])
        mde = float(data["mde"])
        alpha = float(data.get("alpha", 0.05))
        power = float(data.get("power", 0.80))

        ok, msg = validate_inputs(baseline_cr, mde)
        if not ok:
            return jsonify({"error": msg}), 400

        n = calculate_sample_size(baseline_cr, mde, alpha, power)
        return jsonify({"sample_size_per_group": n, "total": n * 2})

    @app.route("/api/analyze", methods=["POST"])
    def api_analyze():
        """Анализирует результаты A/B теста."""
        data = request.get_json()
        conv_a = int(data["conv_a"])
        n_a = int(data["n_a"])
        conv_b = int(data["conv_b"])
        n_b = int(data["n_b"])

        result = compare_proportions(conv_a, n_a, conv_b, n_b)
        uplift = calculate_uplift(result["cr_a"], result["cr_b"])
        interpretation = interpret_result(result["p_value"], uplift)

        return jsonify({**result, "uplift_pct": round(uplift, 2), "interpretation": interpretation})
    ...
    pass  # Flask не установлен — модуль используется как библиотека
